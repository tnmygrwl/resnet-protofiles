...
def normalized_conv_layers(conv_params, level, branch, prev_top, activation=True):
        ...
        name = f'{level}_branch{branch}'
        activation_name = f'res{name}'
...
def bottleneck_layer_set(
        ...
        if sublevel_naming == 'letters' and num_bottlenecks <= 26:
                ...
                sublevel_names = ['a'] + [f'b{str(i)}' for i in range(1, num_bottlenecks)]
...
def resnet(variant='50'): # Currently supports 50, 101, 152
        ...
        network_str = data_layer(f'ResNet-{variant}')
...
def main():
        for net in ('18', '34', '50', '101', '152'):
                with open(f'ResNet_{net}_train_val.prototxt', 'w') as fp:
                        fp.write(resnet(net))
